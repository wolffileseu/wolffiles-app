...
q3bsp = function(gl) {
    // gl initialization
    this.gl = gl;
    // Enable 32-bit index buffers for large maps
    var uint32ext = gl.getExtension("OES_element_index_uint");
    if (!uint32ext) { console.warn("OES_element_index_uint not supported - large maps may render incorrectly"); }
    this.onload = null;
    this.onbsp = null;
    ...
    this.worker = new Worker('/js/bsp-viewer/q3bsp_worker.js?v=' + Date.now());
...
q3bsp.prototype.showLoadStatus = function() {
    var loading = document.getElementById("bsp-loading");
    if (loading) loading.style.display = "flex";
};

q3bsp.prototype.onLoadStatus = function(message) {
    var el = document.getElementById("bsp-progress-text");
...
q3bsp.prototype.buildLightmaps = function(size, lightmaps) {
    if (!lightmaps || lightmaps.length === 0) { console.log("No lightmaps, keeping white default"); q3glshader.init(this.gl, this.lightmap); return; }
    var gl = this.gl;
    ...
    for(var i = 0; i < lightmaps.length; ++i) {
        if (i === 0) { var arr = new Uint8Array(lightmaps[i].bytes); console.log("buildLM debug: size=" + lightmaps[i].width + "x" + lightmaps[i].height + " first bytes:", arr[0], arr[1], arr[2], arr[3], arr[4], arr[5]); }
        if (i === 0) { var arr = new Uint8Array(lightmaps[i].bytes); console.log("buildLM debug: size=" + lightmaps[i].width + "x" + lightmaps[i].height + " first bytes:", arr[0], arr[1], arr[2], arr[3], arr[4], arr[5]); }
        gl.texSubImage2D(
