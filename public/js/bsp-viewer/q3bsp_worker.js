...
importScripts('/js/bsp-viewer/binary-file.js');
importScripts('/js/bsp-viewer/gl-matrix-min.js');
...
q3bsp.load = function(url, tesselationLevel, errorCallback) {
    ...
    request.addEventListener("load", function () {
        try { q3bsp.parse(new BinaryFile(request.response), tesselationLevel); } catch(e) { console.error("Parse error:", e.message, e.stack); postMessage({type:"status", message:"Parse error: " + e.message}); }
    }, false);
...
q3bsp.parse = function(src, tesselationLevel) {
    ...
    if(header.tag != 'IBSP' || ( header.version != 46 && header.version != 47 )) {
        postMessage({
            type: 'status',
    ...
    q3bsp.bspVersion = header.version;
    ...
    console.log("About to read lightmaps, lump 14:", header.lumps[14]);
    var lightmaps = q3bsp.readLightmaps(header.lumps[14], src);
    console.log("Lightmaps read OK, count:", lightmaps ? lightmaps.length : "null");
    ...
    visBuffer = visData ? visData.buffer : null;
    visSize = visData ? visData.size : 0;
...
q3bsp.readLightmaps = function(lump, src) {
    ...
    for(var i = 0; i < count; ++i) {
        ...
        if (i === 0) { console.log("LM debug: first 12 bytes:", elements[0], elements[1], elements[2], elements[3], elements[4], elements[5], elements[6], elements[7], elements[8], elements[9], elements[10], elements[11]); }
        if (i === 0) { console.log("LM debug: first 12 bytes:", elements[0], elements[1], elements[2], elements[3], elements[4], elements[5], elements[6], elements[7], elements[8], elements[9], elements[10], elements[11]); }
        lightmaps.push({
            x: xOffset, y: yOffset,
...
q3bsp.readFaces = function(lump, src) {
    var isET = (q3bsp.bspVersion === 47);
    var faceSize = 104;
    var faceCount = lump.length / faceSize;
    var faces = [];
...
q3bsp.compileMap = function(verts, faces, meshVerts, lightmaps, shaders, tesselationLevel) {
    ...
    for(var i = 0; i < faces.length; ++i) {
        ...
        if(face.type==1 || face.type==2 || face.type==3) {
            ...
            if(!lightmap) {
                lightmap = lightmaps[0] || { x: 0, y: 0, xScale: 1, yScale: 1 };
            }
    ...
    for(var i = 0; i <  shaders.length; ++i) {
        ...
        if(shader.faces.length > 0) {
            shader.indexOffset = indices.length * 4; // Offset is in bytes (32-bit indices)
            
            for(var j = 0; j < shader.faces.length; ++j) {
                var face = shader.faces[j];
                face.indexOffset = indices.length * 4;
...
q3bsp.checkVis = function(visCluster, testCluster) {
    ...
    if (!visBuffer) { return true; } var visSet = visBuffer[i];
