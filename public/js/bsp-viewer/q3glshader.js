...
// BasisLoader removed - using PNG/JPG only
...
// PNG/JPG variant - adapted for Wolffiles.eu
q3glshader.loadTextureUrlImg = function(gl, stage, url, onload) {
    ...
    var customBase = window.bspCustomTextureBase || '';
    var cleanUrl = url.replace(/\.png$/, '').replace(/\.tga$/, '').replace(/\.jpg$/, '');
    var tryUrls = [];
    if (customBase) {
        tryUrls.push(customBase + '/' + cleanUrl + '.jpg');
        tryUrls.push(customBase + '/' + cleanUrl + '.png');
    }
    tryUrls.push(q3bsp_base_folder + '/' + cleanUrl + '.jpg');
    tryUrls.push(q3bsp_base_folder + '/' + cleanUrl + '.png');
    tryUrls.push(q3bsp_base_folder + '/' + url);
    var tryIndex = 0;
    function tryNext() {
        if (tryIndex >= tryUrls.length) return;
        image.onerror = function() { tryIndex++; tryNext(); };
        image.src = tryUrls[tryIndex];
    }
    tryNext();
}
if (true) { // Always use PNG/JPG
