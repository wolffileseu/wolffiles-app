<x-layouts.app :title="__('messages.register')">
    <div class="max-w-md mx-auto px-4 py-12">
        <div class="bg-gray-800 rounded-lg border border-gray-700 p-8">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-300 mb-1">{{ __('messages.name') }}</label>
                ...
                <div class="mb-6">
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-300 mb-1">{{ __('messages.confirm_password') }}</label>
            ...
            @if(config('services.discord.client_id'))
                ...
                <a href="{{ route('auth.discord.redirect') }}"
                   class="w-full flex items-center justify-center space-x-2 bg-[#5865F2] hover:bg-[#4752C4] text-white py-3 rounded-lg font-medium transition-colors">
                    ...
                    <span>{{ __('messages.register_discord') }}</span>
                </a>
            @endif

            <p class="text-center text-gray-400 text-sm mt-6">
                {{ __('messages.have_account') }}
                <a href="{{ route('login') }}" class="text-amber-400 hover:underline">{{ __('messages.login') }}</a>
            </p>
        </div>
    </div>
</x-layouts.app>
